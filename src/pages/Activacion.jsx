...
export default function Activacion() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--c-bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 20px', boxSizing: 'border-box' }}>
      <div style={{ width: '100%', maxWidth: 480, animation: 'fadeIn 0.4s ease' }}>

        {/* Logo PIDOGO */}
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ fontSize: 42, marginBottom: 12 }}>🎉</div>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: 'var(--c-text)', margin: '0 0 8px' }}>
            ¡Bienvenido a Pidoo!
          </h1>
          <p style={{ fontSize: 15, color: 'rgba(0,0,0,0.55)', margin: 0, lineHeight: 1.5 }}>
            <strong style={{ color: 'var(--c-text)' }}>{restaurante?.nombre}</strong> ya está en la plataforma.
          </p>
        ...
        {(restaurante?.logo_url || restaurante?.banner_url || restaurante?.nombre) && (
          <div style={{ background: 'rgba(0,0,0,0.04)', borderRadius: 16, overflow: 'hidden', marginBottom: 20, border: '1px solid rgba(0,0,0,0.08)' }}>
            {restaurante?.banner_url ? (
              ...
              <div style={{ width: '100%', height: 80, background: 'linear-gradient(135deg, rgba(185,28,28,0.3), rgba(185,28,28,0.1))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 11, color: 'rgba(0,0,0,0.3)', fontWeight: 600 }}>Sin banner — puedes añadirlo en Ajustes</span>
              </div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px' }}>
              <div style={{ width: 52, height: 52, borderRadius: 14, background: 'rgba(0,0,0,0.1)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: 15, color: 'var(--c-text)' }}>{restaurante?.nombre}</div>
        ...
        <div style={{ background: 'rgba(0,0,0,0.04)', borderRadius: 16, padding: 20, marginBottom: 20, border: '1px solid rgba(0,0,0,0.08)' }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--c-text)', margin: '0 0 4px' }}>Completa tu perfil</h3>
          <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.35)', margin: '0 0 16px' }}>Puedes editar todo esto más tarde en Ajustes.</p>

          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'rgba(0,0,0,0.45)', marginBottom: 6, display: 'block' }}>
        ...
        <button
          ...
          style={{
            ...
            background: activando ? 'rgba(185,28,28,0.4)' : 'var(--c-primary)',
